...
FILLERS: dict[str, set[str]] = {
    ...
    "hi": {"अं", "अम्म", "हम्म", "मतलब", "वो", "यानी", "तो", "हाँ", "matlab", "yaani", "woh", "haan", "hmm", "um"},
    "ta": {"அது", "வந்து", "அப்புறம்", "ம்ம்", "vandhu", "adhu", "appuram", "hmm"},
    "te": {"అంటే", "అది", "ఏంటంటే", "ante", "adi", "hmm"},
    "bn": {"মানে", "ইয়ে", "মানে", "mane", "iye", "hmm"},
...
def disfluency_features(tokens: list[Token] | None, language: str | None) -> dict[str, float]:
    nan = float("nan")
    if not tokens:
        return {"filler_per_min": nan, "repetition_per_min": nan, "restart_per_min": nan, "disfluency_per_min": nan}
    ...
    repetitions = sum(1 for a, b in zip(words, words[1:], strict=False) if a == b and a not in lexicon)
    restarts = sum(1 for w in words if w.endswith("-") and len(w) > 1)
    restarts += sum(
        1
        for i in range(len(words) - 3)
        if words[i : i + 2] == words[i + 2 : i + 4][:0] + words[i : i + 2] and words[i] != words[i + 1]
        and words[i + 2] == words[i] and words[i + 3] != words[i + 1]
    )
